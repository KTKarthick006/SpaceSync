...
const send = async (to, subject, html) => {
  ...
  try {
    await transporter.sendMail({
      from: process.env.MAIL_FROM || "SpaceSync <noreply@spacesync.com>",
      to,
      subject,
      html,
    });
...
export const mailBookingCreated = (b) => {
  send(
    b.user.email,
    "Booking Request Received — SpaceSync",
    base(`
...
export const mailCancelledByAdmin = (b) => {
  send(
    b.user.email,
    "Booking Cancelled by Admin — SpaceSync",
    base(`
